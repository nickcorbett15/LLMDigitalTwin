...
export const GptChatbot = ()  => {
  ...
  const handleSendMessage = () => {
    if (input.trim()) {
      ...
      const newMessages: GptMessage [] =  [...messages, { role: 'user', content: input.trim() }];
      ...
      setTimeout(async () => {
        ...
        const url = `${baseUrl}/chat-to-openai?message=${encodeURIComponent(message)}&history=${encodeURIComponent(history)}`;
        fetch(url)
